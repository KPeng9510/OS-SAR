...
from model.temperature_prediction import *
...
def get_parser():
    ...
    parser.add_argument(
        '--weights',
        default='/pfs/work7/workspace/scratch/fy2374-train/acmmm_and_aaai/aaai/checkpoints/CTRGCN_crossview_mm_mmd_r5/runs-93-18228.pt',
        help='the weights for network initialization')
    parser.add_argument(
        '--weights_upper',
        default='/pfs/work7/workspace/scratch/fy2374-train/acmmm_and_aaai/aaai/checkpoints/CTRGCN_crossview_mm_mmd_r5/runs-upper93-18228.pt',
        help='the weights for network initialization')
    parser.add_argument(
        '--weights_lower',
        default='/pfs/work7/workspace/scratch/fy2374-train/acmmm_and_aaai/aaai/checkpoints/CTRGCN_crossview_mm_mmd_r5/runs-lowwer93-18228.pt',
        help='the weights for network initialization')
    parser.add_argument(
        '--ignore-weights',
        type=str,
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.logitsmodel =  LogitsTemPredNet().cuda()
        self.mse_loss = torch.nn.MSELoss(size_average=None, reduce=True, reduction='mean').cuda()
    ...
    def load_model(self):
        ...
        self.logitsmodel =  LogitsTemPredNet()
        
        print(self.model)
        self.loss = nn.CrossEntropyLoss().cuda(output_device)
        #self.model = torch.nn.DataParallel(self.model)
        #self.logitsmodel = torch.nn.DataParallel(self.logitsmodel)
        ...
        self.logitsmodel = self.logitsmodel.cuda()
        if self.arg.weights:
    ...
    def load_optimizer(self):
        if self.arg.optimizer == 'SGD':
            self.optimizer = optim.SGD(
                [*self.model.parameters()] + [*self.model_lowerbody.parameters()] + [*self.model_upperbody.parameters()] + [*self.logitsmodel.parameters()],
                lr=self.arg.base_lr,
                momentum=0.9,
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (datalist, label, index) in enumerate(process):
            ...
            output_upper, rep2 = self.model_upperbody(data_upper)
            output_lower, rep3 = self.model_lowerbody(data_lower)

            t1, t2, t3 = self.logitsmodel(rep1, rep2, rep3)
            #print(t1)
            #print(t2)
            #print(t3)
            t1, t2, t3 = t1 + 1, t2 + 1, t3 + 1
            loss_logits = self.mse_loss(t1, torch.ones_like(t1).float().cuda()) + self.mse_loss(t2, torch.ones_like(t2).float().cuda()) * self.mse_loss(t3, torch.ones_like(t3).float().cuda())
            loss = self.loss(output/t1, label) + self.loss(output_lower/t3, label) + torch.mean(self.loss(output_upper/t2, label)) + 0.1*torch.mean(self.mmd_loss(output, output_lower)) + 0.1*torch.mean(self.mmd_loss(output, output_upper)) + 0.1*torch.mean(self.mmd_loss(output_upper, output_lower)) #+ loss_logits
            # backward
        ...
        if save_model:
            ...
            state_dict_logits = self.logitsmodel.state_dict()
            weights_logits = OrderedDict([[k.split('module.')[-1], v.cpu()] for k, v in state_dict_logits.items()])
            ...
            torch.save(weights_logits, self.arg.model_saved_name + '-logits' + str(epoch+1) + '-' + str(int(self.global_step)) + '.pt')
    ...
    def eval(self, epoch, save_score=False, loader_name=['test_seen', 'test_unseen'], wrong_file=None, result_file=None, num_class=None):
        ...
        joints_train_rep = []
        joints_train_out = []
        joints_train_ood_rep = []
        joints_train_ood_out = []
        ...
        for batch_idx, (datalist, label,index) in enumerate(process):
            with torch.no_grad():
                ...
                label = label.long().cuda(self.output_device)
                output, rep1 = self.model(data)
                output_lower, rep2 = self.model_lowerbody(data_lower)
                output_upper, rep3 = self.model_upperbody(data_upper)
                ...
                for i in range(rep2.shape[0]):
                    ...
                    bones_train_out.append(output_lower[i].cpu().numpy())
                for i in range(rep3.shape[0]):
                    vels_train_rep.append(rep3[i].cpu().numpy()/LA.norm(rep3[i].cpu().numpy()))
                    vels_train_out.append(output_upper[i].cpu().numpy())
        ...
        for batch_idx, (datalist, label,index) in enumerate(process):
            with torch.no_grad():
                ...
                for i in range(rep2.shape[0]):
                    ...
                    bones_test_seen_out.append(output_lower[i].cpu().numpy())
                for i in range(rep3.shape[0]):
                    vels_test_seen_rep.append(rep3[i].cpu().numpy()/LA.norm(rep3[i].cpu().numpy()))
                    vels_test_seen_out.append(output_upper[i].cpu().numpy())
            #if batch_idx > 1:
            #    break                                
        ...
        for batch_idx, (datalist, label,index) in enumerate(process):
            with torch.no_grad():
                ...
                for i in range(rep2.shape[0]):
                    ...
                    bones_test_unseen_out.append(output_lower[i].cpu().numpy())
                for i in range(rep3.shape[0]):
                    vels_test_unseen_rep.append(rep3[i].cpu().numpy()/LA.norm(rep3[i].cpu().numpy()))
                    vels_test_unseen_out.append(output_upper[i].cpu().numpy())
            #if batch_idx > 1:
            #    break


        dicty = {'joints_train_rep':joints_train_rep, 'joints_train_out':joints_train_out, 'train_label':train_label, 'bones_train_rep':bones_train_rep,
        'bones_train_out':bones_train_out, 'vels_train_rep':vels_train_rep, 'vels_train_out':vels_train_out, 'joints_test_seen_rep':joints_test_seen_rep,
        'joints_test_seen_out':joints_test_seen_out, 'bones_test_seen_rep':bones_test_seen_rep, 'bones_test_seen_out':bones_test_seen_out,
        'vels_test_seen_rep':vels_test_seen_rep, 'vels_test_seen_out':vels_test_seen_out, 'test_seen_label':test_seen_label, 'joints_test_unseen_rep':joints_test_unseen_rep,
        'joints_test_unseen_rep':joints_test_unseen_rep, 'joints_test_unseen_out':joints_test_unseen_out, 'bones_test_unseen_rep':bones_test_unseen_rep,
        'joints_test_unseen_out':joints_test_unseen_out, 'bones_test_unseen_rep':bones_test_unseen_rep, 'bones_test_unseen_out':bones_test_unseen_out, 
        'vels_test_unseen_rep':vels_test_unseen_rep, 'vels_test_unseen_out':vels_test_unseen_out, 'test_unseen_label':test_unseen_label,
         }

        import pickle as pkl
        f = open('feature_test.pkl', 'wb')
