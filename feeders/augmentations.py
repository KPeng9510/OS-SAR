    ...
    if flip_prob < 0.5:
        ...
        return out
         ...
         Corruption = np.array([
                           ...
                           [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)],
                           [random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1)] ])
         temp = np.dot(np.transpose(temp,(1, 2, 3, 0)), Corruption)
         temp = np.transpose(temp,(3, 0, 1, 2))
         out[:,:,joint_indicies,:] = torch.FloatTensor(temp)
         return out
...
def pose_augmentation(input_data):
        ...
        result =  np.dot(np.transpose(temp_data,(1, 2, 3, 0)),Shear.transpose())
        output = np.transpose(result,(3, 0, 1, 2))
