...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.tpl_loss = losses.TripletMarginLoss().cuda()
        #self.logitsmodel =  LogitsTemPredNet().cuda()
        #self.mse_loss = torch.nn.MSELoss(size_average=None, reduce=True, reduction='mean').cuda()
        self.miner = miners.MultiSimilarityMiner(epsilon=0.1).cuda()
        loss = losses.ContrastiveLoss(pos_margin=0, neg_margin=1).cuda()
        self.loss_cbm_consistency = losses.CrossBatchMemory(loss, 256, memory_size=1024, miner=None).cuda()
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (datalist, label, index) in enumerate(process):
            ...
            data_lower = torch.cat([data_lower, data_lower2], 0)
            data_upper = torch.cat([data_upper, data_upper2], 0)'''
            timer['dataloader'] += self.split_time()
            # forward
            output_all, rep1_all = self.model(data)
            #print(data_upper.shape)
            output_upper_all, rep2_all = self.model_upperbody(data_upper)
            output_lower_all, rep3_all = self.model_lowerbody(data_lower)
            #print(rep1_all.shape)
            #print(rep2_all.shape)
            #print(rep3_all.shape)
            #loss_cbm =  self.loss_cbm_consistency(rep1_all/torch.norm(rep1_all, dim=-1).unsqueeze(-1).repeat(1, rep1_all.shape[-1]), label) +  self.loss_cbm_consistency(rep2_all/torch.norm(rep2_all, dim=-1).unsqueeze(-1).repeat(1, rep2_all.shape[-1]), label) +  self.loss_cbm_consistency(rep3_all/torch.norm(rep3_all, dim=-1).unsqueeze(-1).repeat(1, rep3_all.shape[-1]), label)
            '''z1, z2 = rep1_all[:bs],  rep1_all[bs:]
            z1_upper, z2_upper = rep2_all[:bs],  rep2_all[bs:]
            z1_lower, z2_lower = rep3_all[:bs],  rep3_all[bs:]
            ...
            loss_c = on_diag + 0.0051 * off_diag


            c_upper = z1_upper.T@z2_upper
            c_upper.div_(bs)
            on_diag_upper = torch.diagonal(c_upper).add_(-1).pow_(2).sum()
            off_diag_upper = off_diagonal(c_upper).pow_(2).sum()
            loss_c_upper = on_diag_upper + 0.0051 * off_diag_upper


            c_lower = z1_lower.T@z2_lower
            c_lower.div_(bs)
            on_diag_lower = torch.diagonal(c_lower).add_(-1).pow_(2).sum()
            off_diag_lower = off_diagonal(c_lower).pow_(2).sum()
            loss_c_lower = on_diag_lower + 0.0051 * off_diag_lower

            loss_barlows = 0.1*loss_c + 0.1*loss_c_upper + 0.1*loss_c_lower'''
            #
            ...
            loss = self.loss(output_all, label) + self.loss(output_lower_all, label) + torch.mean(self.loss(output_upper_all, label)) + 0.1*torch.mean(self.mmd_loss(output_all, output_lower_all)) + 0.1*torch.mean(self.mmd_loss(output_all, output_upper_all)) + 0.1*torch.mean(self.mmd_loss(output_upper_all, output_lower_all)) #+ loss_cbm #+ loss_logits
            # backward
    ...
    def eval(self, epoch, save_score=False, loader_name=['test_seen', 'test_unseen'], wrong_file=None, result_file=None, num_class=None):
        ...
        for batch_idx, (datalist, label,index) in enumerate(process):
            with torch.no_grad():
                ...
                #t1, t2, t3 = t1+1, t2+1, t3+1
                output = (output + output_lower + output_upper)/3
                #output = torch.stack(output).cuda()
                output = torch.nn.functional.softmax(output, dim = -1)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if (epoch % self.arg.eval_interval == 0) and (epoch>55):
