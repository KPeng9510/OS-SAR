...
class Processor():
    ...
    def eval_uosr(self, y_true, y_pred):
        ...
        fpr95 = fpr[operation_idx]  # FPR when TPR at 95%
        return auroc, aupr, fpr95
    def get_embedding_uncertainty(self, data, label):
        torch.set_printoptions(threshold=10000)
        pros = self.prototypes # 40,C,T,V
        N,C,T,V = pros.shape
        #calculate the temporal distance
        pros_temporal = pros.mean(-1)
        d_tem = []
        cosine_distance = torch.nn.CosineEmbeddingLoss(margin=0.0, size_average=False, reduce=False, reduction='none')
        for ind, (x, l) in enumerate(zip(data, label)):
            cosdises = torch.mean((x.mean(-1).view(-1,C*T) - pros_temporal[l].view(-1,C*T))**2,-1) 
            #cosdises = cosine_distance(x.mean(-1).view(-1,C*T).unsqueeze(0), pros_temporal[l].unsqueeze(0).view(-1,C*T), torch.ones(1).cuda()).mean(-1)
            #cosdises = (cosdises - torch.min(cosdises))/torch.max(cosdises)
            #print(cosdises)
            #cosdises = torch.nn.functional.normalize(cosdises, dim=0) # 40
            #print(cosdises)
            d_tem.append(cosdises)
        d_tem = torch.stack(d_tem) # B, 40
        #print(d_tem)
        #calculate the spatial distance
        spa_spatial = pros.mean(-2)
        d_spa = []
        cosine_distance = torch.nn.CosineEmbeddingLoss(margin=0.0, size_average=False, reduce=False, reduction='none')
        for ind, (x, l) in enumerate(zip(data, label)):
            cosdises = torch.mean((x.mean(-2).view(-1,C*V) - spa_spatial[l].view(-1,C*V))**2,-1) 
            #cosdises = cosine_distance(x.mean(-2).view(-1,C*V).unsqueeze(0), spa_spatial.view(-1,C*V).unsqueeze(0), torch.ones(1).cuda()).mean(-1)
            #cosdises = torch.nn.functional.normalize(cosdises, dim=0) # 40
            #cosdises = (cosdises - torch.min(cosdises))/torch.max(cosdises)
            d_spa.append(cosdises)
        d_spa = torch.stack(d_spa) # B, 40
        return d_tem, d_spa
    def epoch_wise_prototypes(self,):
        f_train = []
        f_ood = []
        all_features = [[] for i in range(10)]
        prototypes = []
        process = tqdm(self.data_loader['train'], ncols=40)
        features_id = [[] for i in range(10)]
        features_ow = [[] for i in range(10)]
        for batch_idx, (odata, label, index) in enumerate(process):
            with torch.no_grad():
                data = odata.float().cuda(self.output_device)
                label = label.long().cuda(self.output_device)
                output, rep = self.model(data)
                N,C,T,V = rep.shape
            for k in range(data.shape[0]):
                if torch.max(torch.nn.functional.softmax(output[k],-1))>0.9:
                    f_train.append(rep[k].mean(-1).mean(-1))
                    all_features[label[k]].append(rep[k])
                    features_id[label[k]].append(rep[k])
                else:
                    features_ow[label[k]].append(rep[k])
            if batch_idx>100:
                break

        '''f = open('fe/f_train.pkl', 'wb')
        pkl.dump(obj=f_train, file=f)
        f.close()'''

        distance_id = []
        distance_ow = []
        for i in range(10):
            prototypes.append(torch.mean(torch.stack(all_features[i], 0), 0))
        prototypes = torch.stack(prototypes,0)
  
        for i in range(10):
            for x in features_id[i]:
                cosdises = torch.mean((x.unsqueeze(0).repeat(10, 1,1,1).mean(-1).view(-1,C*T) - prototypes.mean(-1).view(-1,C*T))**2,-1)
                cosdises = (cosdises - torch.min(cosdises))
                distance_id.append(torch.mean(cosdises, -1))
            for x in features_ow[i]:
                cosdises = torch.mean((x.unsqueeze(0).repeat(10, 1,1,1).mean(-1).view(-1,C*T) - prototypes.mean(-1).view(-1,C*T))**2,-1)
                cosdises = (cosdises - torch.min(cosdises))
                distance_ow.append(torch.mean(cosdises, -1))
        #print(torch.mean(torch.stack(distance_id)))
        threshold = torch.mean(torch.stack(distance_ow))
    ...
    def eval(self, epoch, save_score=False, loader_name=['test_seen', 'test_unseen'], wrong_file=None, result_file=None, num_class=None):
        ...
        for batch_idx, (datalist, label,index) in enumerate(process):
            with torch.no_grad():
                ...
                output_lower = torch.nn.functional.softmax(output_lower, dim = -1)
                output_upper = torch.nn.functional.softmax(output_upper, dim = -1)
                probab_j, predicted_j = torch.max(output, -1)
                probab_lower, predicted_lower = torch.max(output_lower, -1)
                probab_upper, predicted_upper = torch.max(output_upper, -1)
                ...
                '''for k in range(data.shape[0]):
                    ...
                    pl = probab_lower[k]
                    pu = probab_upper[k]
                    if pj > 0.6 and pl > 0.6:
                        output[k] =  output[k]
                    else:
                        output[k] = (output[k] + output_lower[k] + output_upper[k])/3'''
                #print(probab_j - probab_lower)

                output = (output + output_lower)/2
